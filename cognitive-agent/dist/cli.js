...
const global_agent_1 = require("./core/global-agent");
...
program
    ...
    try {
        ...
        const agent = new global_agent_1.GlobalCognitiveAgent(config);
        await agent.start();
...
program
    ...
    .action(async () => {
    try {
        const { MCPHostDiscovery } = await Promise.resolve().then(() => __importStar(require('./core/host-discovery')));
        ...
        logger_1.logger.error('❌ Failed to discover hosts:', error);
        process.exit(1);
    }
});
/**
...
program
    ...
    try {
        ...
        logger_1.logger.error('❌ Failed to configure agent:', error);
        process.exit(1);
    }
});
/**
...
async function loadConfig(options) {
    ...
    if (options.config) {
        try {
            ...
            logger_1.logger.warn(`⚠️ Could not load config file ${options.config}, using defaults`);
        }
    }
    else {
